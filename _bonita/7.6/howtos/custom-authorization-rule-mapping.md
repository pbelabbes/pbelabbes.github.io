...
Authorization Rule Mapping is a set of rules used to allow connected user to access [page and form]({{"bonita/7.6/application-and-process-design/page-and-form-development/page-and-form-development-overview" | relative_url}}). An empty list grants access to user. A non empty list allows access if all rules return true to their `isAllowed` method. Those rules grant or not access to the connected user for:
