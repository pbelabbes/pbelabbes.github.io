...
A client application therefore needs to poll regularly to check when the asynchronous work unit is finished, or write an [event handler]({{"bonita/7.6/development/bonita-engine-architecture/services/event-handlers" | relative_url}}) in order to be notified.
