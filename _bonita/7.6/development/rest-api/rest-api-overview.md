...
Access to the Web REST API depends on [REST API authorization]({{"bonita/7.6/installation/advanced-bonita-platform-installation/security-and-authentication/rest-api-authorization" | relative_url}}) settings.
...
1. [Start a new case with variables]({{"bonita/7.6/development/rest-api/bpm-api" | relative_url}}#case): Provide a form for the user to enter initial data. Then call the method to start a new case using the values entered by the user to initialize some variables. The engine will start the execution of the process. Depending on the design of your process, there might then be some human tasks available for the end user.
2. [List the pending tasks for a user]({{"bonita/7.6/development/rest-api/bpm-api" | relative_url}}#human-task): Retrieve a list of available human tasks for the logged in user. When the user selects a task to do, you can display the corresponding form. It can be an external form or a Bonita form that can be accessed by url.
3. [Update variables and execute a task]({{"bonita/7.6/development/rest-api/bpm-api" | relative_url}}#activity): If your application is using an external form, update the values of the variables in your process. 
...
4. [Handle tasks in error]({{"bonita/7.6/development/rest-api/bpm-api" | relative_url}}#connector-instance): Get a list of tasks that are in the failed state, and then replay each task by doing three steps: get the list of failed connectors, reset the state of failed connectors and replay the failed task.
...
You can create [Rest API Extensions]({{"bonita/7.6/development/rest-api/rest-api-extensions" | relative_url}}) to extend the Rest API by adding missing ressources (not provided by the Rest API). 
...
If you are searching for business data using a custom query, there must be a [count query in the BDM]({{"bonita/7.6/application-and-process-design/data/define-and-deploy-the-bdm" | relative_url}}). If there is no count query, results from a custom query on business data cannot be paged properly (the header Content-Range will be absent). 
...
* s: search on name or search indexes. The matching policy depends on the configuration of [word-based search]({{"bonita/7.6/development/engine-api/using-list-and-search-methods" | relative_url}}). 
