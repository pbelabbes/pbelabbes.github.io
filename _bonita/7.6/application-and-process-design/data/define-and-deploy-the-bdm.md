...
When you are ready to go to production, you need to [configure a database and datasources for business data]({{"bonita/7.6/installation/basic-bonita-platform-installation/database-configuration" | relative_url}}) in your production environment.
...
Business objects can be combined using composition or aggregation relationships. They are managed using standard create, read, update, delete actions. A business object can be read directly using the [Engine Java API](http://documentation.bonitasoft.com/javadoc/api/{{page.version}}/index.html) specifying the Java DAO, or using the [REST API]({{"bonita/7.6/development/rest-api/bdm-api" | relative_url}}). To read a business object in a Groovy expression, use the DAO. A business object can be updated only in a process, using an operation.
...
{% alert warning %}
:fa-exclamation-triangle: **Warning:** Before deploying the business data model, you must [pause]({{"bonita/7.6/bonita-portal-administration/process-maintenance/pause-and-resume-bpm-services" | relative_url}}) the BPM services, so that the update can be performed without affecting ongoing processes. 
**Important:** You are recommended [Back up your platform]({{"bonita/7.6/installation/back-up-bonita-bpm-platform" | relative_url}}) and database before deploying a new BDM in case of problems during the update.
{% endalert %}
...
7. Click **Pause**.
8. When the BPM services are paused, go to the **Business Data Model** menu (and, for Efficiency and Performance editions, choose **Definition**).
...
When you design a process, you specify the variables to be used in the process. These can include variables that are instances of business objects defined in the BDM. You [define a business object variable]({{"bonita/7.6/application-and-process-design/data/specify-data-in-a-process-definition" | relative_url}}) in Bonita Studio in a similar way to defining other process variables.
...
Be aware that a poor implementation of a custom REST API accessing BDM objects can lead to poor performance results. See the [best practice]({{"bonita/7.6/howtos/bdm-in-rest-api" | relative_url}}) on this matter.