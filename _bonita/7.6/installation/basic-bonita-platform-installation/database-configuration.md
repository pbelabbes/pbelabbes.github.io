...
- We recommend that you configure a different database schema if your project uses [business data]({{"bonita/7.6/application-and-process-design/data/define-and-deploy-the-bdm" | relative_url}}).

Bonita supports MySQL, PostgreSQL, SQL Server, and Oracle RDBMSs.
You can find the list of RDBMSs versions supported on the [supported database list]({{"bonita/7.6/installation/basic-bonita-platform-installation/hardware-and-software-requirements" | relative_url}}) page.
...
To use Bonita on another RDBMS, please use a bundle or set up a [standalone server]({{"bonita/7.6/installation/basic-bonita-platform-installation/deploy-bundle" | relative_url}}).
{% endalert %}
...
If the same SQL user is used with the [migration tool]({{"bonita/7.6/installation/migration/migrate-from-an-earlier-version-of-bonita-bpm" | relative_url}}), then this user needs such grants.
{% endalert %}
...
Now that your databases are created and customized to work with Bonita, you are ready to configure Bonita bundles, either the [Tomcat bundle]({{"bonita/7.6/installation/basic-bonita-platform-installation/tomcat-bundle" | relative_url}}) or the [WildFly bundle]({{"bonita/7.6/installation/basic-bonita-platform-installation/wildfly-bundle" | relative_url}}).
