...
The tool migrates your platform (_bonita_home_ folder and the database). You cannot [change edition]({{"bonita/7.6/installation/upgrade-from-community-to-a-subscription-edition" | relative_url}}) while migrating. If you are running a
...
On the other hand, if you have customized your configuration, you will have to use the [platform setup tool]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf) to send your customized configuration files to the database where configuration is stored, for versions 7.3 and above.
{% endalert %}
...
If you are using a custom Look & Feel, [export]({{"bonita/7.6/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) it before migrating.
Then after the migration is complete, [export the default Look & Feel]({{"bonita/7.6/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) from the new version,
modify your custom Look & Feel to be compatible with the new definition, and with the [recommendations for form footers]({{"bonita/7.6/bonita-portal-administration/look-feel/creating-a-new-look-feel" | relative_url}}).
Then [import]({{"bonita/7.6/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) your updated custom Look & Feel into Bonita Portal.
...
Therefore, you must do a [backup of your configuration files]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf) before starting the migration.  
...
Therefore, it is strongly recommended to do a [backup of your database]({{"bonita/7.6/installation/back-up-bonita-bpm-platform" | relative_url}}) before starting the migration.
{% endalert %}
...
1. If you use a custom Look & Feel, [export]({{"bonita/7.6/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) it, and then [restore the default Look & Feel]({{"bonita/7.6/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}).
...
1. **IMPORTANT:**
[Back up your platform]({{"bonita/7.6/installation/back-up-bonita-bpm-platform" | relative_url}}) and database in case of problems during migration.
...
1. [Configure the bundleto use the migrated database]({{"bonita/7.6/installation/basic-bonita-platform-installation/database-configuration" | relative_url}}).
...
Please refer to the guide on updating the configurationfile using the [platform setup tool]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf)
...
1. <a id="compound-permission-migration" /> **If your Bonita version is 7.4 or above before migrating, you can skip this point.**In the case where deployed resources have required dedicated [authorizations to use the REST API]({{"bonita/7.6/bonita-portal-administration/resources/resource-management" | relative_url}}#permissions), these authorizations are not automatically migrated.
...
1. Configure License:
    ...
    you need to put a new license in the database: see [Platform configuration]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf) for further details.
    There is below a Linux example:
    ```
    cd setup
    ...
    If there is no valid license in the `./platform_conf/licenses/`, these 2 pages will help you to request and install a new one:
      ...
      * [Platform configuration]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf)
...
If you did not set the default Look & Feel before migration and you cannot log in, you need to [restore the default Look & Feel]({{"bonita/7.6/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) using a REST client or the Engine API.
