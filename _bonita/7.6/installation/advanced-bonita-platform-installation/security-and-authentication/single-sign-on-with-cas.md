...
1. The CAS implementation relies on JAAS, and is defined in the BonitaAuthentication module of the JAAS configuration file.  
   Set the Java system property `java.security.auth.login.config` in the Tomcat startup script to point to the JAAS configuration file, [`TOMCAT_HOME/server/conf/jaas-standard.cfg`]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}). 
...
To troubleshoote SSO login issues, you need to increase the [log level]({{"bonita/7.6/development/logging" | relative_url}}) to `ALL` in order for errors to be displayed in the log files (by default, they are not).
...
If you are using an LDAP service and the [LDAP synchronizer]({{"bonita/7.6/application-and-process-design/organization/ldap-synchronizer" | relative_url}}) to manage your user data, you can continue to do this and use CAS. The LDAP synchronizer user must be registered in CAS. 
...
* [Bonita REST API]({{"bonita/7.6/development/rest-api/rest-api-overview" | relative_url}})
