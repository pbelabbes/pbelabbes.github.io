...
**Important notes:**  
* This documentation applies to an existing and working Bonita installation (see the [installation instructions]({{"bonita/7.6/installation/bonita-bpm-installation-overview" | relative_url}})).
* In order to have functioning Active Directory/LDAP authentication, the user login (username) must exist both in the LDAP directory and in the Bonita database (user password is checked against the LDAP server but user information is read from Bonita database).  We recommend that you use the [LDAP synchronizer]({{"bonita/7.6/application-and-process-design/organization/ldap-synchronizer" | relative_url}}) to create Bonita users in a Bonita database.
...
order to activate Active Directory/LDAP authentication the service implementation needs to be changed. To do this, edit [`bonita-tenant-sp-custom.properties`]({{"bonita/7.6/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}).
