...
{% alert danger %}
:fa-exclamation-triangle: **Important:** A task is not automatically assigned to the user who accesses the task form. There must first be a [REST API call to assign the task to the user]({{"bonita/7.7/development/rest-api/bpm-api" | relative_url}}). Otherwise, the user will not be able to execute the task.
{% endalert %}
...
Go to [Accessing Bonita Portal and forms by URL 6.5 documentation]({{"bonita/7.7/development/bonita-bpm-portal-urls" | relative_url}}) for more information about this.
