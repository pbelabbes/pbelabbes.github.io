...
Pages are [exported. imported, modified, and deleted]({{"bonita/7.7/bonita-portal-administration/resources/resource-management" | relative_url}}) as resources in Bonita Portal. 

## Custom page definition

A page resource has the general [resource definition]({{"bonita/7.7/bonita-portal-administration/resources/resource-management" | relative_url}}). 
...
If you are using [REST API authorization]({{"bonita/7.7/installation/advanced-bonita-platform-installation/security-and-authentication/rest-api-authorization" | relative_url}}) and your custom page is an HTML page using the Web REST API, 
...
You can find examples of the default resources in [`resources-permissions-mapping.properties`]({{"bonita/7.7/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}).
...
To view an example page, publish the page to a [custom profile]({{"bonita/7.7/bonita-portal-administration/user-profiles/custom-profiles" | relative_url}}) or an [application]({{"bonita/7.7/bonita-portal-administration/applications" | relative_url}}) so that you can view it in the portal. To view the source of an example, [export the custom page]({{"bonita/7.7/bonita-portal-administration/resources/resource-management" | relative_url}}#export).
...
To enable custom page debug mode, edit [`console-config.properties`]({{"bonita/7.7/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}) and set `custom.page.debug` to `true`.
...
If your page is viewed in a custom profile or in an application, you will have access facilities for [the portal API]({{"bonita/7.7/development/rest-api/rest-api-overview" | relative_url}}).

you will be able to access the portal API using the following path: `../API/{API name}/{resource name}`

### Theme access

If your page is viewed in an application, you will have access facilities for [the application theme]({{"bonita/7.7/bonita-portal-administration/applications" | relative_url}}).
