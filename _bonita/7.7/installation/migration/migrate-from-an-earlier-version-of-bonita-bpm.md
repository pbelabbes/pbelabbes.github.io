...
The tool migrates your platform (_bonita_home_ folder and the database). You cannot [change edition]({{"bonita/7.7/installation/upgrade-from-community-to-a-subscription-edition" | relative_url}}) while migrating. If you are running a
...
If you are using a custom Look & Feel, [export]({{"bonita/7.7/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) it before migrating.
Then after the migration is complete, [export the default Look & Feel]({{"bonita/7.7/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) from the new version,
modify your custom Look & Feel to be compatible with the new definition, and with the [recommendations for form footers]({{"bonita/7.7/bonita-portal-administration/look-feel/creating-a-new-look-feel" | relative_url}}).
Then [import]({{"bonita/7.7/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) your updated custom Look & Feel into Bonita Portal.
...
Therefore, you must do a [backup of your configuration files]({{"bonita/7.7/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf) before starting the migration.  
...
Therefore, it is strongly recommended to do a [backup of your database]({{"bonita/7.7/installation/back-up-bonita-bpm-platform" | relative_url}}) before starting the migration.
{% endalert %}
...
1. If you use a custom Look & Feel, [export]({{"bonita/7.7/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) it, and then [restore the default Look & Feel]({{"bonita/7.7/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}).
...
1. **IMPORTANT:**
[Back up your platform]({{"bonita/7.7/installation/back-up-bonita-bpm-platform" | relative_url}}) and database in case of problems during migration.
...
1. [Configure the bundleto use the migrated database]({{"bonita/7.7/installation/basic-bonita-platform-installation/database-configuration" | relative_url}}).
...
Please refer to the guide on updating the configurationfile using the [platform setup tool]({{"bonita/7.7/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf)
...
1. Configure License:
    ...
    you need to put a new license in the database: see [Platform configuration]({{"bonita/7.7/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf) for further details.
    There is below a Linux example:
    ```
    cd setup
    ...
    If there is no valid license in the `./platform_conf/licenses/`, these 2 pages will help you to request and install a new one:
      ...
      * [Platform configuration]({{"bonita/7.7/installation/basic-bonita-platform-installation/BonitaBPM_platform_setup" | relative_url}}#update_platform_conf)
...
If you did not set the default Look & Feel before migration and you cannot log in, you need to [restore the default Look & Feel]({{"bonita/7.7/bonita-portal-administration/look-feel/managing-look-feel" | relative_url}}) using a REST client or the Engine API.
