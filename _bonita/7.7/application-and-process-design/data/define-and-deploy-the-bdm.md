...
When you are ready to go to production, you need to [configure a database and datasources for business data]({{"bonita/7.7/installation/basic-bonita-platform-installation/database-configuration" | relative_url}}) in your production environment.
...
{% alert warning %}
:fa-exclamation-triangle: **Warning:** Before deploying the business data model, you must [pause]({{"bonita/7.7/bonita-portal-administration/process-maintenance/pause-and-resume-bpm-services" | relative_url}}) the BPM services, so that the update can be performed without affecting ongoing processes. 
**Important:** You are recommended [Back up your platform]({{"bonita/7.7/installation/back-up-bonita-bpm-platform" | relative_url}}) and database before deploying a new BDM in case of problems during the update.
{% endalert %}
...
When you design a process, you specify the variables to be used in the process. These can include variables that are instances of business objects defined in the BDM. You [define a business object variable]({{"bonita/7.7/application-and-process-design/define-access-control-on-business-objects/specify-data-in-a-process-definition" | relative_url}}) in Bonita Studio in a similar way to defining other process variables.
