...
Bonita Studio includes an environment for testing a process locally, before deployment. You can test the process without connectors by clicking **_Debug_**, test each connector independently using teh connector wizard **_Test_** button, then run the process by clicking **_Debug_** to test it with connectors. You need to [configure]({{"bonita/7.7/application-and-process-design/process-configuration/process-configuration-overview" | relative_url}}) the process before testing it locally.
...
When testing is complete, the processes are ready to be deployed in Bonita Portal and the application is ready to be put into production. You need to [configure]({{"bonita/7.7/application-and-process-design/process-configuration/process-configuration-overview" | relative_url}}) each process for deployment before putting it into production.
...
You can [manage your organization in Bonita Portal]({{"bonita/7.7/bonita-portal-administration/organization-in-bonita-portal/organization-in-bonita-bpm-portal-overview" | relative_url}}) for a production system.   
While you are in the testing phase, you can [manage the organization in Bonita Studio]({{"bonita/7.7/application-and-process-design/organization/organization-management-in-bonita-bpm-studio" | relative_url}}).
