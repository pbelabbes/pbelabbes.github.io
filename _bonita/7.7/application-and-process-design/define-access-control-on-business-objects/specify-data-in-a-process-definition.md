...
See also:  
[Data types and defining a variable]({{"bonita/7.7/application-and-process-design/data/data-handling-overview" | relative_url}})  
[Complex data types]({{"bonita/7.7/application-and-process-design/define-access-control-on-business-objects/create-a-complex-data-type" | relative_url}})  
[Data handling in a form]({{"bonita/7.7/application-and-process-design/page-and-form-development/ui-designer/variables" | relative_url}})  
...
There are also a number of variables that are provided by default in a process, such as the id of the current user. They are available through the [expression editor]({{"bonita/7.7/application-and-process-design/define-access-control-on-business-objects/expressions-and-scripts" | relative_url}}).
...
A process or application can use data that is stored in an external database and accessed using a [database connector]({{"bonita/7.7/application-and-process-design/connectivity/database/list-of-database-connectors" | relative_url}}). For example, in a process for requesting annual leave, there could be a step that uses a connector to retrieve the number of days available for an employee and stores this in a transient variable. This variable is then
...
You can also use data stored in external information system components, such as a CRM document storage system. These systems are also accessed using a [connector]({{"bonita/7.7/application-and-process-design/connectivity/connectivity-overview" | relative_url}}).
...
Note that since 7.0, Business data is now available. For a detailed explanation, see the [Business Data overview]({{"bonita/7.7/application-and-process-design/data/define-and-deploy-the-bdm" | relative_url}}).

You can also define additional [complex data types]({{"bonita/7.7/application-and-process-design/define-access-control-on-business-objects/create-a-complex-data-type" | relative_url}}) as Java objects or as XML objects.
...
Note: if you add Business data, only add it at the pool/lane level. See this tutorial as a guide for the next steps[How to add Business data to a process]({{"bonita/7.7/application-and-process-design/data/define-and-deploy-the-bdm" | relative_url}}).
...
You can define a variable whose data type is Java object. Before you define the variable, you must [create the data type definition]({{"bonita/7.7/application-and-process-design/define-access-control-on-business-objects/create-a-complex-data-type" | relative_url}}).
...
Before you define the variable, you must [create the XML schema]({{"bonita/7.7/application-and-process-design/define-access-control-on-business-objects/create-a-complex-data-type" | relative_url}}) that contains the data type definition.  
