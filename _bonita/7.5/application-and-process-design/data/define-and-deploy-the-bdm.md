...
When you are ready to go to production, you need to [configure a database and datasources for business data]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/database-configuration" | relative_url}}) in your production environment.
...
{% alert warning %}
:fa-exclamation-triangle: **Warning:** Before deploying the business data model, you must [pause]({{"bonita/7.5/bonita-bpm-portal-administration/process-maintenance/pause-and-resume-bpm-services" | relative_url}}) the BPM services, so that the update can be performed without affecting ongoing processes. 
**Important:** You are recommended [Back up your platform]({{"bonita/7.5/installation/back-up-bonita-bpm-platform" | relative_url}}) and database before deploying a new BDM in case of problems during the update.
{% endalert %}
