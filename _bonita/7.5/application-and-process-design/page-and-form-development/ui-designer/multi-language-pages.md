...
Each page has a [localization asset]({{"bonita/7.5/application-and-process-design/page-and-form-development/ui-designer/assets" | relative_url}}) that contains the keys and the translations for all strings and all languages that the page supports, one section by language.  
...
The code that specifies a language in `localization.json` must be the same as the code in the `.po` files used to [add the language to the Portal]({{"bonita/7.5/bonita-bpm-portal-administration/bonita-bpm-portal-interface/languages" | relative_url}}).
...
This tutorial explains how to create a multi-language version of the Travel Tool application page from the [getting started tutorial]({{"bonita/7.5/application-and-process-design/application-and-process-design-overview/getting-started-tutorial" | relative_url}}), adding french and russian text to the page, initially developped in US-english. It assumes that you have already created the Travel Tool page following the instructions in the [getting started tutorial]({{"bonita/7.5/application-and-process-design/application-and-process-design-overview/getting-started-tutorial" | relative_url}}).
...
Before you can view a page in a language, that language must be supported for Bonita BPM web applications. French is one of the default languages supported, so you just need to add support for Russian. To do this, [add Russian from the Community translations to your Bonita BPM Portal]({{"bonita/7.5/bonita-bpm-portal-administration/bonita-bpm-portal-interface/languages" | relative_url}}). Make sure that you add both the Community and Subscription files. Then restart your Bonita BPM Studio, start the Portal, and change the language to Russian to check that it is available.
...
In the custom widget editor, use the the _uiTranslate_ filter or the _ui-translate_ directive to indicate the content to translate.   Then add the strings to the localization.json file of each page, layout and forms using it. See [Custom widgets section]({{"bonita/7.5/application-and-process-design/page-and-form-development/ui-designer/custom-widgets" | relative_url}}) for more information.
...
To put a multi-language page into production in an application, follow the same steps as for a single-language page: [upload the page to the Portal]({{"bonita/7.5/bonita-bpm-portal-administration/resources/resource-management" | relative_url}}) and then [add it to the application]({{"bonita/7.5/bonita-bpm-portal-administration/applications" | relative_url}}). You can follow the steps for [building the application]({{"bonita/7.5/application-and-process-design/application-and-process-design-overview/getting-started-tutorial" | relative_url}}) from the getting started tutorial.
