---
layout : doc
title : "Database creation and customization to work with Bonita BPM"
version : "7.5"
categories:
  - Installation
...
- We recommend that you configure a different database schema if your project uses [business data]({{"bonita/7.5/application-and-process-design/data/define-and-deploy-the-bdm" | relative_url}}).

Bonita BPM supports MySQL, PostgreSQL, SQL Server, and Oracle RDBMSs.
You can find the list of RDBMSs versions supported on the [supported database list]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/hardware-and-software-requirements" | relative_url}}) page.
...
To use Bonita BPM on another RDBMS, please use a bundle or set up a [standalone server]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/deploy-bundle" | relative_url}}).
{% endalert %}
...
Steps 1 and 2 are detailed further down this page, steps 3 and 4 are part of the configuration part of [Tomcat bundle]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/tomcat-bundle" | relative_url}}) and [WildFly bundle]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/wildfly-bundle" | relative_url}}) pages.
...
If the same SQL user is used with the [migration tool]({{"bonita/7.5/installation/migration/migrate-from-an-earlier-version-of-bonita-bpm" | relative_url}}), then this user needs such grants.
{% endalert %}
...
Now that your databases are created and customized to work with Bonita BPM, you are ready to configure Bonita BPM bundles, either the [Tomcat bundle]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/tomcat-bundle" | relative_url}}) or the [WildFly bundle]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/wildfly-bundle" | relative_url}}).
