...
It is also possible to [use gzip compression]({{"bonita/7.5/installation/advanced-bonita-bpm-platform-installation/use-gzip-compression" | relative_url}}) on your application server to improve performance.
...
This section deals with performance impact of your choice of [Engine access mode]({{"bonita/7.5/development/engine-api/engine-api-overview" | relative_url}}).
...
This method of accessing the Bonita BPM capabilities is not yet integrated as an engine service but exists as a web application service accessed using the [Web REST API]({{"bonita/7.5/development/rest-api/rest-api-overview" | relative_url}}).  
...
Before you read this, make sure you are familiar with the engine [execution sequence, states, and transactions]({{"bonita/7.5/development/bonita-bpm-engine-architecture/execution-sequence-states-and-transactions" | relative_url}}).
...
The work service is configured in [`bonita-tenant-community-custom.properties`]({{"bonita/7.5/installation/basic-bonita-bpm-platform-installation/BonitaBPM_platform_setup" | relative_url}}).
```
...
With this in mind, [define the log level for technical logs, queriable logs and archives]({{"bonita/7.5/installation/advanced-bonita-bpm-platform-installation/set-log-and-archive-levels" | relative_url}}).
...
Bonita BPM Engine uses the [Scheduler service]({{"bonita/7.5/development/bonita-bpm-engine-architecture/engine-architecture-overview" | relative_url}}) to trigger jobs in a recurrent manner.
