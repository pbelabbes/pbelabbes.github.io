...
In this deployment you must configure the JVM containing the client Bonita BPM Portal to use the remote connection to the engine. To do so see [configuring the connection to a remote engine]({{"bonita/7.5/development/engine-api/configure-client-of-bonita-bpm-engine" | relative_url}}#client_config).
