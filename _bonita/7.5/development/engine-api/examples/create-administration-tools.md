...
You need to be familiar with building a Maven project with Bonita BPM as a dependency. You also need to be familiar with using the Bonita BPM Engine APIs. If this is not the case, follow this tutorial.
The [Engine API overview]({{"bonita/7.5/development/engine-api/engine-api-overview" | relative_url}}) contains a list of the APIs with a short description of each one. For details of the methods available in the Bonita BPM Engine APIs, see the [Javadoc](http://documentation.bonitasoft.com/javadoc/api/{{page.version}}/index.html). 

This example uses the Bonita BPM Engine Java APIs but there is also a [REST API]({{"bonita/7.5/development/rest-api/rest-api-overview" | relative_url}}).
