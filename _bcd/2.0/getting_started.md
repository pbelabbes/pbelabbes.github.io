...
In order to ease your environment setup, we provide a Docker image called **[BCD Controller image]({{"bcd/2.0/bcd-services-and-scenarios/bcd_controller" | relative_url}})** which contains all required dependencies and tools for BCD.  
...
{% alert info %}
You may also start a BCD Controller container using [Docker Compose](https://docs.docker.com/compose/) as described in the [BCD Controller image]({{"bcd/2.0/bcd-services-and-scenarios/bcd_controller" | relative_url}}) detailed documentation.
{% endalert %}
...
If you consider deploying to Amazon Web Services (AWS) platform, first read the [AWS prerequisites tutorial]({{"bcd/2.0/howtos/manage-bonita-stacks/aws_prerequisites" | relative_url}}) which describes how to configure AWS for BCD.
...
{% alert info %}
Read the **[BCD Command-line interface]({{"bcd/2.0/bcd-services-and-scenarios/bcd_cli" | relative_url}})** page for further information about the BCD CLI usage.
{% endalert %}
...
{% alert info %}
Read the **[BCD Scenario reference]({{"bcd/2.0/bcd-services-and-scenarios/scenarios" | relative_url}})** page for a comprehensive description of scenario variables.
{% endalert %}
